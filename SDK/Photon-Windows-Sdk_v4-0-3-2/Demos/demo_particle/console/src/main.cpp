...
int main(int argc, char* argv[])
{
	...
	Client lbc(lbl, appId, appVersion, JString(L"NVC")+GETTIMEMS(), gUseTcp);
...
//	lbc.setLimitOfUnreliableCommands(15);
	...
	while(true)
	{
		...
		if(k >= '0' && k <= '9')
		{
			...
			{
				Console::get().writeLine(JString(L"setting interest group to send updates to to group nr ") + g + L" ...");
				gSendGroup = g;
				lbl.setSendGroup(g);
			}
			info(lbc);
